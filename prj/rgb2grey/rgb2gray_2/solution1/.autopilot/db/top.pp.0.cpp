...
typedef uint8_t gray_pixel;

typedef ap_axiu<32, 0, 0, 0> rgb;
typedef ap_axiu<8, 0, 0, 0> gray;
void top(hls::stream<rgb>& src, hls::stream<rgb>& dst, int rows, int cols);
# 2 "../../src/rgb2gray.h" 2
...
__attribute__((sdx_kernel("top", 0))) void top(hls::stream<rgb>& src, hls::stream<gray>& dst, int rows, int cols) {
#line 16 "/root/Xilinx/rgb2grey/prj/rgb2grey/rgb2gray_2/solution1/csynth.tcl"
...
#pragma HLS INTERFACE s_axilite port=return bundle=CTRL
 ...
 rgb_pixel rgb_tmp[16];
  gray_pixel gray_tmp[16];
  rgb rgb_pkt[16];
  VITIS_LOOP_16_1: for (int i = 0; i < rows; i++) {
    VITIS_LOOP_17_2: for (int j = 0; j < cols; j += 16) {
#pragma HLS PIPELINE II=4
 VITIS_LOOP_19_3: for (int k = 0; k < 16; k++) {
#pragma HLS UNROLL
 rgb_pkt[k] = src.read();
      }
      VITIS_LOOP_23_4: for (int k = 0; k < 16; k++) {
#pragma HLS UNROLL
 rgb_tmp[k].a = 0;
        rgb_tmp[k].r = (rgb_pkt[k].data >> 16) & 0xFF;
        rgb_tmp[k].g = (rgb_pkt[k].data >> 8) & 0xFF;
        rgb_tmp[k].b = rgb_pkt[k].data & 0xFF;
      }

      VITIS_LOOP_31_5: for (int k = 0; k < 16; k++) {
#pragma HLS UNROLL
 gray_tmp[k] = rgb2gray_pixel(rgb_tmp[k]);
      }

      VITIS_LOOP_36_6: for (int k = 0; k < 16; k++) {
#pragma HLS UNROLL
 gray g;
        g.data = gray_tmp[k];
        dst.write(g);
